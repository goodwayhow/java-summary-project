...
import hanlp.utility.MathTools;
...
public class WordNet
{
     ...
     */
	private LinkedList<Vertex> vertexes[]; //這裡是看wordNet裡面幾個row ,而每個row又可能會放一些節點
    ...
    public WordNet(char[] charArray)
    {
        ...
        vertexes[0].add(Vertex.newB());//空白行 0:[]
        vertexes[vertexes.length - 1].add(Vertex.newE());//空白行  length-1:[]
        size = 2;
    }
    /*
    ...
    public void add(int line, Vertex vertex)
    {
        for (Vertex oldVertex : vertexes[line])
        {
        	...
        	if (oldVertex.realWord == vertex.realWord) return; //2016/11/02 我覺得改這樣才對，判斷有無重複字詞
        }
...
*/
    ...
    public void addAll(List<Vertex> vertexList)
    {
        ...
        {
            add(i, vertex);
            i += vertex.realWord.length();//每個節點(詞)的字數都不一樣長
        }
    }
     ...
     * @return
     */
    public Vertex getFirst(int line) //每一row 的第一個 vertex
    {
     ...
     * @return
     */
    public Vertex get(int line, int length)//把某一列的全部節點 ,個別跟vertex比較長度是否一樣  
    {
    ...
    {
        Vertex[] vertexes = new Vertex[size];  //System.out.println("節點個數"+size); //每一個都要算(包含[])
        int i = 0;
        for (List<Vertex> vertexList : this.vertexes)
        {
            for (Vertex v : vertexList)
            {
                v.index = i;    // 设置每個節點的 id(index)
                vertexes[i++] = v;
            }
        }
    ...
    public Graph toGraph()
    {
        ...
        {
            List<Vertex> vertexListFrom = vertexes[row];//把wordNet裡面的每一row 節點放進去vertexListFrom
            //
            System.out.println(vertexListFrom);
            //
